...
<div class="articles container-fluid p-0 d-flex flex-column align-items-center col-12" style="height: 100%">
    ...
    <div class="col-9 p-4">
        ...
        <form id="article_form" action="{{ route('create_article') }}" method="POST" class="mb-5">
                ...
                <div class="col-3">
                    <select class="form-select" aria-label="form-select" name="report_id" id="report_id">
                        ...
                        @foreach ($reports as $report)
                            <option {{ $article["report_id"] == $report['id'] ? 'selected' : '' }} value="{{ $report['id'] }}"> 
                                {{ $report['year'] }} 
                            </option>
                        @endforeach
                    </select>
            ...
            @if (isset($article["path_to_additional_content"]))
                <div class="row" id="existing_additional_file_holder">
                    <div class="col-6 pt-3">
                        ✔️
                        <a id="existing_additional_file" name="existing_additional_file" href="{{ Storage::url($article["path_to_additional_content"]); }}">
                            Файл статті</a>
                        <button id="existing_additional_file_remove_button" class="btn btn-danger ms-1">
                            Видалити
                        </button>
                    </div>
                </div>
...
@section('javascript_import')
    ...
    <script defer src="{{ asset('js/update_article.js') }}"></script>
@show
